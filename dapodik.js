...
app.post('/data-siswa', isAuthenticated,
    [
        ...
        //? Validasi NoKK untuk duplikasi dan panjang digit
        body('nokk').custom(async (value) => {
            const duplikat = await Siswa.findOne({ nokk: value });
            if (duplikat) {
                throw new Error('NoKK sudah terdaftar!');
            }
            return true;
        }).isLength({ min: 16, max: 16 }).withMessage('NoKK harus terdiri dari 16 digit angka').
...
app.put('/data-siswa', isAuthenticated,
    [
        check('tingkat').isIn(['X', 'XI', 'XII']).withMessage('Tingkat tidak valid'),
        check('rombel').custom((value, { req }) => {
            const validRombel = req.body.tingkat === 'X'
                ? ['PPLG 1', 'PPLG 2']
                : ['RPL 1', 'RPL 2'];
            if (!validRombel.includes(value)) {
                throw new Error('Rombel tidak sesuai dengan tingkat');
            }
            return true;
        }),
